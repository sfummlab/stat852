...
# ####################################################
  ...
  ###############################################################
  library(glmnet)
  multinom <- glmnet(x=as.matrix(x.r), y=y.r, s = 0, type.measure = "class",  family="multinomial", parallel = TRUE)
  multinom
  plot(multinom)

  multinom.pred.train <- predict(object=multinom, newx=as.matrix(x.r),type="class")
  multinom.pred.test <- predict(multinom, newx=as.matrix(x.p), type="class")
  (multinom.misclass.train <- mean(ifelse(multinom.pred.train == y.r, yes=0, no=1)))
  (multinom.misclass.test <- mean(ifelse(multinom.pred.test == y.p, yes=0, no=1)))
  multinom.miss.train[i] <- multinom.misclass.train
  multinom.miss.test[i] <-  multinom.misclass.test
