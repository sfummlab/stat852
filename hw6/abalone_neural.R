...
for(i in 1:20)
{
  ...
  ppr.iter <- ppr(data=abelone[which(abelone$set==1),], Rings ~ Length + Height +  Diameter + Whole + 
                    Shucked + Viscera + Shell + sex.m + sex.f, nterms=2, optlevel=3)
  ...
  MSPE.earth.best[i] <- earth.list[which.min(grsq)]
  
  
  #----------------------------------Neural Network--------------------------------#
  library(nnet)
  nn <- nnet(y=y.1, x=x.1.scaled, linout=TRUE, size=2, decay=0.01, maxit=500, trace=FALSE)
  
  pred.nn <- predict(nn,newdata = x.2.scaled)
  
  MSPE.nn[i] <- mean((y.2 - pred.nn)^2)
  ...
  abalone.rf <- randomForest(x.1, y.1, importance=TRUE, ntree=1000, mtry=4,nodesize=20, keep.forest=TRUE)
  randf.mspe <- mean((predict(abalone.rf,x.2)-y.2)^2)
  
  
   MSPE.randf[i] <- randf.mspe
  ...
  abelone.boost <- gbm(data=data.frame(x.1,Rings=y.1), Rings ~ ., distribution="gaussian", 
                       n.trees=12000, interaction.depth=7,verbose=FALSE, shrinkage=0.001,
                       bag.fraction=0.25, cv.folds=0,n.cores=16)
  
  
  
   MSPE.gbm [i] <- mean((y.2- predict(abelone.boost,newdata=data.frame(x.2),n.trees=8000))^2)
  ...
  split_MSPE <- cbind(MSPE.ols, MSPE.lasson.min, MSPE.lasson.1se,  MSPE.allsub.bic,  MSPE.allsub.aic,
                      MSPE.step, MSPE.bma,gam.mspe, ppr.mspe, MSPE.rpart.prune[i], MSPE.rpart[i],MSPE.earth.best[i],MSPE.nn[i],MSPE.gbm[i],MSPE.randf[i])
  ...
  MSPE.earth.1.re[i] <-  MSPE.earth.1[i]/ min_MSPE
  MSPE.earth.2.re[i] <-  MSPE.earth.2[i] / min_MSPE
  MSPE.earth.3.re[i] <- MSPE.earth.3[i] / min_MSPE
  MSPE.earth.best.re[i] <- MSPE.earth.best[i] / min_MSPE
  
  
  MSPE.nn.re[i] <- MSPE.nn[i] / min_MSPE

  
   MSPE.gbm.re[i] <- MSPE.gbm[i] / min_MSPE
...
MSE <- data.frame(ols = MSPE.sum.ols, lasso.min = MSPE.sum.lasso.min,  lasso.1se = MSPE.sum.lasso.1se, allsub.bic = MSPE.sum.allsub.bic,
                  ...
                  tree.prune = MSPE.rpart.prune, 
                  earth_best = MSPE.earth.best, nnet = MSPE.nn, randf = MSPE.randf, gbm = MSPE.gbm)
...
MSE.re <- data.frame(ols = MSPE.sum.ols.re, lasso.min = MSPE.sum.lasso.min.re,  lasso.1se = MSPE.sum.lasso.1se.re, allsub.bic = MSPE.sum.allsub.bic.re,
                     ...
                     earth_best_re = MSPE.earth.best.re, nnet = MSPE.nn.re, randf = MSPE.randf.re, gbm = MSPE.gbm.re)
MSE <- sqrt(MSE)
